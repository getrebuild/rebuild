...
const step2_mapping = () => {
  ...
  $.get(`/app/data/data-imports/check-file?file=${$encode(_Config.file)}`, (res) => {
    $btn.button('reset')
    if (res.error_code > 0) {
...
const step3_import = () => {
  ...
  RbAlert.create($L('请再次确认导入选项和字段映射。开始导入吗？'), {
    confirm: function () {
      this.disabled(true)
      $.post('/app/data/data-imports/import-submit', JSON.stringify(_Config), (res) => {
        if (res.error_code === 0) {
          this.hide()
          step3_import_show()
          import_inprogress = true
...
function _checkUserPrivileges() {
  ...
  $.get(`/app/data/data-imports/check-user?user=${_Config.owning_user}&entity=${_Config.entity}`, (res) => {
    let hasError = []
...
function _renderRepeatFields(entity) {
  ...
  $.get(`/app/data/data-imports/import-fields?entity=${entity}`, (res) => {
    $(res.data).each(function () {
...
class ImportsTraceViewer extends RbAlert {
  ...
  load() {
    $.get(`/app/data/data-imports/import-trace?taskid=${this.props.taskid}`, (res) => {
      if (res.error_code === 0) {
