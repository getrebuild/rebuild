...
class ValueDescription extends ValueComp {
  ...
  renderElement() {
    if (this.state.editMode) {
      ...
      const ps = {
        className: 'form-control-plaintext markdown-body hover',
  ...
  renderViewElement() {
    if (this.state.description) {
      return (
        <div className="form-control-plaintext markdown-body">
          <Md2Html markdown={this.state.description} />
        </div>
      )
    } else {
    ...
    if (data) {
      return (
        <div className={`form-control-plaintext ${useEdit ? 'hover' : ''}`} onClick={() => useEdit && this.setState({ editMode: true })}>
          <a href={`${rb.baseUrl}/app/redirect?id=${data.id}&type=newtab`} title={$L('查看记录')} target="_blank" onClick={(e) => $stopEvent(e)}>
            {data.text}
          </a>
        </div>
      )
