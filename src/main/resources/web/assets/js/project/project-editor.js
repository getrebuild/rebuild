...
$(document).ready(() => {
  ...
  const $btn = $('.J_save').click(() => {
    ...
    if (!data.members) return RbHighbar.create($lang('PlsSelectSome,Members'))
...
class PlanList extends React.Component {
  ...
  render() {
    return (
      <React.Fragment>
        ...
        {this.state.plans && this.state.plans.length === 0 && <p className="text-muted m-3">{$lang('PlsAddSome,ProjectPlan')}</p>}
      </React.Fragment>
    )
  }
  ...
  _handleDelete(planId) {
    const that = this
    RbAlert.create($lang('DeletePlanConfirm'), {
      type: 'danger',
      ...
      confirm: function () {
        ...
        $.post(`/app/entity/record-delete?id=${planId}`, (res) => {
          ...
          if (res.error_code === 0) {
            that.loadPlans()
            RbHighbar.success($lang('SomeDeleted,ProjectPlan'))
          } else RbHighbar.error(res.error_msg)
        })
      },
    })
...
class PlanEdit extends RbFormHandler {
  ...
  render() {
    ...
    return (
      <RbModal title={`${$lang(this.props.id ? 'Modify' : 'Add')}${$lang('ProjectPlan')}`} ref={(c) => (this._dlg = c)} disposeOnHide={true}>
        <div className="form">
          ...
          {this.props.flowNexts && this.props.flowNexts.length > 0 && (
            <div className="form-group row pt-0">
              <label className="col-sm-3 col-form-label text-sm-right">{$lang('FlowNexts')}</label>
