...
$(document).ready(() => {
  ...
  $('.J_whenFilter .btn').on('click', () => {
    if (advFilter) {
      ...
      renderRbcomp(<AdvFilter entity={wpc.sourceEntity} filter={wpc.whenFilter} confirm={saveFilter} title={$L('附加过滤条件')} inModal canNoFilters showPreview showCurrentUser />, function () {
        advFilter = this
      })
    }
  })
  ...
  const $btn = $('.J_save').on('click', () => {
    ...
    if (rb.commercial < 1 && ((when & 512) !== 0 || (when & 1024) !== 0 || (when & 2048) !== 0)) {
