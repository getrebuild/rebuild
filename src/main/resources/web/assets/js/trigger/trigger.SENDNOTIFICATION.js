...
class ContentSendNotification extends ActionContentSpec {
      ...
      <div className="send-notification">
        <form className="simple">
          <div className="form-group row pt-1">
            <label className="col-12 col-lg-3 col-form-label text-lg-right">{$lang('NotifyType')}</label>
            <div className="col-12 col-lg-8 pt-1">
              ...
              <label className="custom-control custom-control-sm custom-radio custom-control-inline mb-1">
                ...
                <span className="custom-control-label">
                  {$lang('Sms')} {this.state.serviceSms === false && `(${$lang('Unavailable')})`}
                </span>
              </label>
            </div>
          </div>
          <div className="form-group row pt-1">
            <label className="col-12 col-lg-3 col-form-label text-lg-right">{$lang('SendToWho')}</label>
          ...
          {this.state.type === 2 && (
            <div className="form-group row pb-1">
              <label className="col-12 col-lg-3 col-form-label text-lg-right">{$lang('EmailSubject')}</label>
              <div className="col-12 col-lg-8">
                <input type="text" className="form-control form-control-sm" ref={(c) => (this._title = c)} maxLength="60" placeholder={$lang('YouHave1Notify')} />
              </div>
            </div>
          )}
          <div className="form-group row pb-1">
            <label className="col-12 col-lg-3 col-form-label text-lg-right">{$lang('Content')}</label>
            <div className="col-12 col-lg-8">
              <textarea className="form-control form-control-sm row3x" ref={(c) => (this._content = c)} maxLength="600"></textarea>
              <p className="form-text">{$lang('NotifyContentTips')}</p>
            </div>
          </div>
        </form>
      </div>
  ...
  buildContent() {
    ...
    if (!_data.sendTo || _data.sendTo.length === 0) {
      RbHighbar.create($lang('PlsSelectSone,SendToWho'))
      return false
    }
    if (!_data.content) {
      RbHighbar.create($lang('SomeNotEmpty,Content'))
      return false
    }
    return _data
  }
