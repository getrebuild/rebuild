...
class ContentSendNotification extends ActionContentSpec {
        ...
        <form className="simple" onSubmit={(e) => e.preventDefault()}>
          ...
          <div>
            {(state.type === 2 || state.type === 1) && (
              <div className={`form-group row pb-1 ${state.type === 1 && 'bosskey-show'}`}>
                <label className="col-12 col-lg-3 col-form-label text-lg-right">{$L('标题')}</label>
                <div className="col-12 col-lg-8">
                  <input type="text" className="form-control form-control-sm" ref={(c) => (this._$title = c)} maxLength="300" placeholder={$L('你有一条新通知')} style={{ maxWidth: '100%' }} />
                </div>
              </div>
            )}
    ...
    if (content) {
      ...
      this.setState(
        ...
        () => {
          ...
          if (content.title) $(this._$title).parents('.bosskey-show').removeClass('bosskey-show')
  ...
  buildContent() {
    if (rb.commercial < 1 && this.state.type === 4) {
      RbAlertFree43.create($L('免费版不支持企业微信群功能 [(查看详情)](https://getrebuild.com/docs/rbv-features)'))
      return false
    }
    if (rb.commercial < 1 && this.state.type === 5) {
      RbAlertFree43.create($L('免费版不支持钉钉群功能 [(查看详情)](https://getrebuild.com/docs/rbv-features)'))
      return false
    }
    if (rb.commercial < 1 && this.state.type === 6) {
      RbAlertFree43.create($L('免费版不支持飞书群功能 [(查看详情)](https://getrebuild.com/docs/rbv-features)'))
      return false
    }
