...
- 空字段无需返回
- 字段列表中标注（必填）的字段在新建时不能为空，请尽量从原文中提取其值
- 标注（只读）的字段不可写入，请勿填写
...
更新已有主记录时，如需删除某条已有明细，在 `$DETAILS$` 中传入该明细的记录 ID 并设置 `delete: true`，可与新增/更新的明细混合提交。新建记录时不能删除明细。
...
- 明细记录 ID 必须由用户提供或从记录数据中获取，不得自行编造
