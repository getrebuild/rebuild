...
public class UserMemory implements Tool {
    ...
    public static final String MEMORY_GUIDANCE =
            "通过 UserMemory 工具为用户维护长期个性化记忆。\n" +
                    ...
                    "- update/delete 前先通过 list 查看现有记忆，使用 list 结果中的序号（no）或记忆内容定位目标条目并向用户确认后再操作\n" +
                    "- 注入的记忆仅供参考，不要主动向用户复述";
    ...
    public Object tool(String arguments) throws Exception {
        ...
        switch (action) {
            ...
            case "update": {
                ...
                if (no == null && StringUtils.isBlank(oldContent)) {
                    throw new KnownToolException("update 操作必须提供 no 或 oldContent 参数（可先通过 list 查看）");
                }
                message = update(user, no, oldContent, content, args.getInteger("level"));
                break;
            }
            case "delete": {
                Integer no = args.getInteger("no");
                String content = args.getString("content");
                if (no == null && StringUtils.isBlank(content)) {
                    throw new KnownToolException("delete 操作必须提供 no 或 content 参数（可先通过 list 查看）");
                }
                message = delete(user, no, content);
                break;
            }
            case "list": {
                message = list(user);
    ...
    /**
     * 按序号或旧内容定位并修改记忆（按内容匹配找到多条取最新一条）
     *
     * @param user
     * @param no 序号（对应 list 结果中的编号）
     * @param oldContent
     * @param content
     * @param level
     * @return
    ...
    private String update(ID user, Integer no, String oldContent, String content, Integer level) {
        ...
        if (no != null) {
            ...
            for (ConfigBean cb : memories) {
                if (StringUtils.isNotBlank(oldContent) && oldContent.equals(cb.getString("content"))) {
                    target = cb;
                    break;
                }
            }
     ...
     * @return
     */
    private String delete(ID user, Integer no, String content) {
        ...
        int deleted = 0;
        for (ConfigBean cb : memories) {
            if (content != null && content.equals(cb.getString("content"))) {
    ...
    /**
     * 按序号定位记忆（序号与 list 结果的展示顺序一致）
     *
     * @param memories
     * @param no
     * @return
     */
    private ConfigBean byNo(ConfigBean[] memories, int no) {
        if (no < 1 || no > memories.length) return null;
        return memories[no - 1];
    }
