				...
				<div class="col-12 col-sm-8">
					...
					<div class="tab-container">
                		<ul class="nav nav-tabs nav-tabs-classic">
							<li class="nav-item"><a class="nav-link" href="form-design">表单布局</a></li>
							<li class="nav-item"><a class="nav-link active" href="#view-design">视图布局</a></li>
						</ul>
...
$(document).ready(function(){
	...
	let btn = $('.J_save').click(function(){
		...
		let _data = { belongEntity:'${entityName}', type:'VIEW', config:JSON.stringify(elements) };
		_data.metadata = { entity:'LayoutConfig', id:config.id || null };
		btn.button('loading');
		$.post('view-update', JSON.stringify(_data), function(res){
	...
	if (data.fieldName == '$LINE$'){
		...
		$('<a href="javascript:;" title="修改分栏名称">[修改]</a>').appendTo(actions).click(function(){
			...
			rb.alter(input, '修改分栏名称', { html: true, confirm: function(){
				this.hide()
				let name = $(this.refs['rbalter']).find('input').val();
				if (name){
