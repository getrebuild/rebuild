    ...
    return <div className="feeds-post">
      <ul className="list-unstyled list-inline mb-1 pl-1" ref={(c) => this._activeType = c}>
        <li className="list-inline-item">
          <a onClick={() => this.setState({ type: 1 })} className={`${type === 1 ? 'text-primary' : ''}`}>动态</a>
        </li>
        <li className="list-inline-item">
          <a onClick={() => this.setState({ type: 2 })} className={`${type === 2 ? 'text-primary' : ''}`}>跟进</a>
        </li>
        {rb.isAdminUser && <li className="list-inline-item">
          <a onClick={() => this.setState({ type: 3 })} className={`${type === 3 ? 'text-primary' : ''}`}>公告</a>
        </li>
        }
      </ul>
      <div className="arrow_box" ref={(c) => this._activeArrow = c}></div>
      <div>
        <FeedsEditor ref={(c) => this._editor = c} type={type} />
      </div>
      <div className="mt-3">
