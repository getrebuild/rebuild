...
class RbFormModal extends React.Component {
          ...
          <div className='modal-dialog'>
            <div className='modal-content'>
              <div className='modal-header modal-header-colored'>
                ...
                {rb.isAdminUser ? <a className='close s' href={rb.baseUrl + '/admin/entity/' + this.state.entity + '/form-design'} title='配置布局' target='_blank'><span className='zmdi zmdi-settings' /></a> : null}
                <button className='close md-close' type='button' onClick={() => this.hide()}><span className='zmdi zmdi-close' /></button>
              </div>
  ...
  renderFromError (message) {
    let error = <div className='alert alert-danger alert-icon mt-5 w-75 mlr-auto'>
      <div className='icon'><i className='zmdi zmdi-alert-triangle' /></div>
      <div className='message' dangerouslySetInnerHTML={{ __html: '<strong>抱歉!</strong> ' + message }} />
    </div>
...
class RbForm extends React.Component {
  ...
  renderFormAction () {
    ...
    let saveBtns = (
      <div className='btn-group dropup btn-space'>
        ...
        <button className='btn btn-primary dropdown-toggle auto' type='button' data-toggle='dropdown'><span className='icon zmdi zmdi-chevron-up' /></button>
  ...
  post (next) {
    ...
    for (let k in this.__FormData) {
      ...
      if (err) { rb.highbar(err); return } else _data[k] = this.__FormData[k].value
    }
    ...
    return (
      <div className='input-group datetime-field'>
        ...
        <div className='input-group-append'>
          <button className='btn btn-primary' type='button' ref='field-value-icon'><i className='icon zmdi zmdi-calendar' /></button>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className='img-field'>
        {this.state.value.map((item) => {
          ...
          return (<span key={'file-' + item}><a title={fileName} className='img-thumbnail img-upload'><img src={itemUrl + '?imageView2/2/w/100/interlace/1/q/100'} /><b title='移除' onClick={() => this.removeItem(item)}><span className='zmdi zmdi-close' /></b></a></span>)
        })}
        <span title='选择图片'>
          <input type='file' className='inputfile' ref='upload-input' id={this.props.field + '-input'} accept='image/*' />
          <label htmlFor={this.props.field + '-input'} className='img-thumbnail img-upload'><span className='zmdi zmdi-image-alt' /></label>
        </span>
    ...
    return (
      <div className='file-field'>
        {this.state.value.map((item) => {
          ...
          return (<div key={'file-' + item} className='img-thumbnail' title={fileName}><i className={'ftype ' + fileIcon} /><span>{fileName}</span><b title='移除' onClick={() => this.removeItem(item)}><span className='zmdi zmdi-close' /></b></div>)
        })}
        <div className='file-select'>
          <input type='file' className='inputfile' ref='upload-input' id={this.props.field + '-input'} />
          <label htmlFor={this.props.field + '-input'} className='btn-secondary'><i className='zmdi zmdi-upload' /><span>选择文件</span></label>
        </div>
        <input ref='field-value' type='hidden' value={this.state.value} />
      </div>
    )
          ...
          <div className='modal-dialog'>
            <div className='modal-content' style={{ width: this.mcWidth + 'px' }}>
              <div className={'modal-body iframe rb-loading ' + (this.state.inLoad == true && 'rb-loading-active')}>
                <iframe className={this.state.isHide ? 'invisible' : ''} src={this.state.showAfterUrl || 'about:blank'} frameBorder='0' scrolling='no' />
                <RbSpinner />
              </div>
            </div>
          </div>
            ...
            <div className='modal-body'>
              <div className='text-center ml-6 mr-6'>
                ...
                <div className='mt-4 mb-3' ref='btns'>
                  ...
                  <button className='btn btn-space btn-danger' type='button' onClick={() => this.confirm()}>删除</button>
                </div>
              </div>
            </div>
          </div>
...
rb.RbViewModal = function (props, subView) {
  ...
  if (subView == true) {
    ...
    let m = renderRbcomp(<RbViewModal url={viewUrl} disposeOnHide id={props.id} subView />)
