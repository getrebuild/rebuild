...
const _renderOption = function (item, idx) {
  ...
  let options = [<option key={`opt-${item.id}`} value={item.id}
    dangerouslySetInnerHTML={{ __html: idx === 0 ? item.text : `${'&nbsp;&nbsp;'.repeat(idx)}-&nbsp;${item.text}` }}
  />]
  if (item.children) {
...
const __findPaths = function (active, push) {
  push.unshift(active.find('>a').text())
...
$(document).ready(() => {
  let clickNav = function (item) {
    ...
    $(paths).each((idx, item) => {
      $(`<li class="breadcrumb-item active">${item}</li>`).appendTo(ol)
      // let active = idx === paths.length - 1
      // let li = $(`<li class="breadcrumb-item ${active ? 'active' : ''}"></li>`).appendTo(ol)
      // if (active) li.text(item)
      // else $(`<a href="#!/">${item}</a>`).appendTo(li)
    })
  }
  ...
  $('.J_add-folder').click(() => renderRbcomp(<FolderEditDlg />))
  $('.J_upload-file').click(() => renderRbcomp(<FileUploadDlg inFolder={currentFolderId} />))
})